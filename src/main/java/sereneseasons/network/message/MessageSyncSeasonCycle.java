...
public class MessageSyncSeasonCycle
        implements IMessage, IMessageHandler<MessageSyncSeasonCycle, IMessage> {
    ...
    public IMessage onMessage(MessageSyncSeasonCycle message, MessageContext ctx) {
        if (ctx.side == Side.CLIENT) {
            ...
            if (playerDimension == message.dimension)
                SeasonHandler.clientSeasonCycleTicks.replace(
                        playerDimension, message.seasonCycleTicks);
        }
