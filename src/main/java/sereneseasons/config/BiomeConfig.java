...
public class BiomeConfig {
    // We use a HashMap for maximum performance as JsonUtil#getOrCreateConfigFile will return a
    // LinkedHashMap
    ...
    public static void init(File configDir) {
        ...
        Map<String, BiomeData> tmpBiomeDataMap =
                JsonUtil.getOrCreateConfigFile(
                        ...
                        "biome_info.json",
                        defaultBiomeData,
                        new TypeToken<Map<String, BiomeData>>() {}.getType());

        if (tmpBiomeDataMap != null && !tmpBiomeDataMap.isEmpty()) {
            // We convert our keys to ResourceLocations here as to avoid calling
            // `ResourceLocation#toString()` everywhere
    ...
    private static void addBlacklistedBiomes(Map<String, BiomeData> map) {
        ...
        for (String biomeName : blacklistedBiomes) {
            if (!map.containsKey(biomeName)) map.put(biomeName, new BiomeData(false, false, false));
            else map.get(biomeName).enableSeasonalEffects = false;
        }
    }
    ...
    private static void addTropicalBiomes(Map<String, BiomeData> map) {
        ...
        for (String biomeName : tropicalBiomes) {
            if (!map.containsKey(biomeName)) map.put(biomeName, new BiomeData(true, true, false));
            else map.get(biomeName).useTropicalSeasons = true;
        }
    }

    private static void addDisabledCropBiomes(Map<String, BiomeData> map) {
        List<String> disabledCropBiomes =
                Lists.newArrayList(
                        "biomesoplenty:crag",
                        "biomesoplenty:wasteland",
                        "biomesoplenty:volcanic_island");
        ...
        for (String biomeName : disabledCropBiomes) {
            if (!map.containsKey(biomeName)) map.put(biomeName, new BiomeData(false, false, true));
            else map.get(biomeName).disableCrops = true;
        }
    }
}
