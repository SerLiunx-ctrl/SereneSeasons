    ...
    protected void loadConfiguration() {
        try {
            addSyncedValue(
                    ...
                    Integer.MAX_VALUE);
            addSyncedValue(
                    SeasonsOption.SUB_SEASON_DURATION,
                    7,
                    TIME_SETTINGS,
                    ...
                    Integer.MAX_VALUE);
            addSyncedValue(
                    SeasonsOption.STARTING_SUB_SEASON,
                    5,
                    TIME_SETTINGS,
                    "The starting sub season for new worlds.  0 = Random, 1 - 3 = Early/Mid/Late"
                        + " Spring, 4 - 6 = Early/Mid/Late Summer, 7 - 9 = Early/Mid/Late Autumn,"
                        + " 10 - 12 = Early/Mid/Late Winter",
                    0,
                    12);
            addSyncedValue(
                    SeasonsOption.PROGRESS_SEASON_WHILE_OFFLINE,
                    true,
                    TIME_SETTINGS,
                    "If the season should progress on a server with no players online");
            addSyncedValue(
                    SeasonsOption.ADVANCE_SEASON_WHILE_SLEEPING,
                    true,
                    TIME_SETTINGS,
                    "If the season should progress if all players are sleeping");
            ...
            SereneSeasons.logger.error(
                    "Serene Seasons has encountered a problem loading seasons.cfg", e);
        } finally {
