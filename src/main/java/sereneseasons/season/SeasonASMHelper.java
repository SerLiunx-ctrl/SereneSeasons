...
public class SeasonASMHelper {
    ...
    // Legacy
    public static boolean canSnowAtInSeason(
            World world, BlockPos pos, boolean checkLight, @Nullable ISeasonState seasonState) {
    ...
    public static boolean canSnowAtInSeason(
            ...
            boolean useUnmodifiedTemperature) {
        Biome biome = world.getBiome(pos);
        float temperature = biome.getTemperature(pos);

        if (BiomeConfig.enablesSeasonalEffects(biome)
                && !useUnmodifiedTemperature
        ...
        if (temperature >= 0.15F) {
            return false;
        } else if (biome.getDefaultTemperature() >= 0.15F
                && !ModConfig.seasons.generateSnowAndIce) {
            return false;
        } else if (checkLight) {
            if (pos.getY() >= 0
                    && pos.getY() < 256
    ...
    public static boolean canBlockFreezeInSeason(
            World world, BlockPos pos, boolean noWaterAdj, @Nullable ISeasonState seasonState) {
    ...
    public static float getFloatTemperature(World world, Biome biome, BlockPos pos) {
        ...
        return getFloatTemperature(
                new SeasonTime(SeasonHelper.getSeasonState(world).getSeasonCycleTicks())
                        .getSubSeason(),
                biome,
                pos);
    }
    ...
    public static float getFloatTemperature(SubSeason subSeason, Biome biome, BlockPos pos) {
        ...
        if (!tropicalBiome
                && biome.getDefaultTemperature() <= 0.8F
                && BiomeConfig.enablesSeasonalEffects(biome)) {
            switch (subSeason) {
                default:
                    break;
    ...
    public static boolean shouldRenderRainSnow(World world, Biome biome) {
        if (BiomeConfig.usesTropicalSeasons(biome)
                && BiomeConfig.enablesSeasonalEffects(biome)
                && SeasonsConfig.isDimensionWhitelisted(world.provider.getDimension())) {
            Season.TropicalSeason tropicalSeason =
                    SeasonHelper.getSeasonState(world).getTropicalSeason();
