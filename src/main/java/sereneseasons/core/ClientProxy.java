...
public class ClientProxy extends CommonProxy {
    ...
    public void registerItemVariantModel(Item item, String name, int metadata) {
        ...
        Preconditions.checkArgument(
                item != Items.AIR, "Cannot register models for air (" + name + ")");

        ModelLoader.registerItemVariants(item, new ResourceLocation("sereneseasons:" + name));
        ModelLoader.setCustomModelResourceLocation(
                item,
                metadata,
                new ModelResourceLocation(SereneSeasons.MOD_ID + ":" + name, "inventory"));
    }
    ...
    public void registerBlockSided(Block block) {
        if (block instanceof ISSBlock) {
            ...
            if (nonRenderingProperties != null) {
                // use a custom state mapper which will ignore the properties specified in the block
                // as
                // being non-rendering
                IStateMapper custom_mapper =
                        (new StateMap.Builder()).ignore(nonRenderingProperties).build();
    ...
    public void registerItemSided(Item item) {
        ...
        if (item.getHasSubtypes()) {
            ...
            for (ItemStack subItem : subItems) {
                ...
                subItemName =
                        subItemName.substring(
                                subItemName.indexOf(".") + 1); // remove 'item.' from the front

                ModelLoader.registerItemVariants(
                        item, new ResourceLocation(SereneSeasons.MOD_ID, subItemName));
                ModelLoader.setCustomModelResourceLocation(
                        item,
                        subItem.getMetadata(),
                        new ModelResourceLocation(
                                SereneSeasons.MOD_ID + ":" + subItemName, "inventory"));
            }
        } else {
            ModelLoader.setCustomModelResourceLocation(
                    item,
                    0,
                    new ModelResourceLocation(
                            SereneSeasons.MOD_ID + ":" + item.delegate.name().getResourcePath(),
                            "inventory"));
        }
    }

    //
    // The below method and class is used as part of Forge 1668+'s workaround for render manager
    // being
    // null during preinit
    //

    private static <E extends Entity> void registerEntityRenderer(
            Class<E> entityClass, Class<? extends Render<E>> renderClass) {
        RenderingRegistry.registerEntityRenderingHandler(
                entityClass, new EntityRenderFactory<E>(renderClass));
    }
