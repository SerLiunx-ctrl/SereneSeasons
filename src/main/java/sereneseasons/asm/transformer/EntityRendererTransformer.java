...
public class EntityRendererTransformer implements IClassTransformer {
    private static final String[] RENDER_RAIN_SNOW_NAMES =
            new String[] {"renderRainSnow", "func_78474_d", "c"};
    private static final String[] ADD_RAIN_PARTICLES_NAMES =
            new String[] {"addRainParticles", "func_78484_h", "q"};

    private static final String[] GET_FLOAT_TEMPERATURE_NAMES =
            new String[] {"getTemperature", "func_180626_a", "a"};
    ...
    private byte[] transformEntityRenderer(byte[] bytes, boolean obfuscatedClass) {
        ...
        for (MethodNode methodNode : classNode.methods) {
            if (ASMHelper.methodEquals(methodNode, RENDER_RAIN_SNOW_NAMES, "(F)V")) {
                ...
                MethodInsnNode targetMethodInsnNode =
                        ASMHelper.getUniqueMethodInsnNode(
                                methodNode,
                                Opcodes.INVOKEVIRTUAL,
                                ObfHelper.unmapType(
                                        obfuscatedClass, "net/minecraft/world/biome/Biome"),
                                CAN_RAIN_NAMES,
                ...
                if (targetMethodInsnNode != null) {
                    ...
                    // Take the world var num from a nearby instruction incase some other mod breaks
                    // it.
                    // Looking at you Optifine!
                    if (methodNode.instructions.get(worldAloadIndex) instanceof VarInsnNode) {
                        worldVarNum =
                                ((VarInsnNode) methodNode.instructions.get(worldAloadIndex)).var;
                    } else {
                        throw new RuntimeException(
                                "Failed to locate world var number whilst replacing biome.canRain()"
                                    + " || biome.getEnableSnow()");
                    }
                    ...
                    ASMHelper.clearNextInstructions(
                            methodNode, methodNode.instructions.get(targetInsnIndex + 1), 3);

                    // Add world argument
                    methodNode.instructions.insertBefore(
                            methodNode.instructions.get(targetInsnIndex - 1),
                            new VarInsnNode(Opcodes.ALOAD, worldVarNum));

                    targetInsnIndex -= 1;

                    int offset = 0;
                    AbstractInsnNode currentInsn =
                            methodNode.instructions.get(targetInsnIndex - offset);
                    ;

                    // Remove any extraneous instructions on this line. This should hopefully help
                    // prevent
                    // other mods
                ...
                targetMethodInsnNode =
                        ASMHelper.getUniqueMethodInsnNode(
                                ...
                                ObfHelper.createMethodDescriptor(
                                        obfuscatedClass, "F", "net/minecraft/util/math/BlockPos"));
                ...
                if (targetMethodInsnNode != null) {
                    ...
                    // Add world argument
                    methodNode.instructions.insertBefore(
                            methodNode.instructions.get(targetInsnIndex - 2),
                            new VarInsnNode(Opcodes.ALOAD, worldVarNum));
        ...
        if (successfulTransformations.size() != 2)
            throw new RuntimeException(
                    "An error occurred transforming EntityRenderer. Applied transformations: "
