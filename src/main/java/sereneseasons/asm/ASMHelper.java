...
public class ASMHelper {
    ...
    public static void clearNextInstructions(MethodNode methodNode, AbstractInsnNode insnNode) {
        Iterator<AbstractInsnNode> iterator =
                methodNode.instructions.iterator(methodNode.instructions.indexOf(insnNode));
    ...
    public static void clearNextInstructions(
            MethodNode methodNode, AbstractInsnNode insnNode, int count) {
        Iterator<AbstractInsnNode> iterator =
                methodNode.instructions.iterator(methodNode.instructions.indexOf(insnNode));
    ...
    public static MethodInsnNode getUniqueMethodInsnNode(
            MethodNode methodNode, int opcode, String owner, String[] names, String desc) {
        List<MethodInsnNode> matchedMethodNodes =
                matchMethodInsnNodes(methodNode, opcode, owner, names, desc);

        if (matchedMethodNodes.isEmpty())
            throw new RuntimeException(
                    "No method instruction node found matching "
                            + owner
                            + " "
                            + names[0]
                            + " "
        ...
        if (matchedMethodNodes.size() > 1)
            LOGGER.warn(
                    "Too many matched instructions were found in "
                            + methodNode.name
                            + " for "
                            + owner
                            + " "
