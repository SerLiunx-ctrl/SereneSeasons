...
/**
 * Constructs efficient data structures to process, store, and give access to data from the
 * FertilityConfig file
 */
public class ModFertility {
    ...
    public static boolean isCropFertile(String cropName, World world, BlockPos pos) {
        ...
        if (BiomeConfig.disablesCrops(biome)) {
            return false;
        } else if (!FertilityConfig.general_category.seasonal_crops
        ...
        if (BiomeConfig.usesTropicalSeasons(biome)) {
            ...
            if (biome.getTemperature(pos) < 0.15F) {
                ...
                // Check if unspecified crops are by default fertile in non-winter, and that it's
                // not winter
    ...
    /**
     * Initializes the crops for a particular season. User's responsibility to match seeds and
     * cropSet to be of the same season (eg. String [] spring_seeds, HashSet springPlants)
     *
    ...
    public static void setupTooltips(ItemTooltipEvent event) {
        ...
        if (FertilityConfig.general_category.crop_tooltips
            ...
            if (seedSeasons.containsKey(name)) {
                ...
                event.getToolTip()
                        .add(I18n.format("tooltip.season_seed.fertile_seasons.line") + ":");
                    ...
                    event.getToolTip()
                            .add(
                                    TextFormatting.LIGHT_PURPLE
                                            + " "
                                            + I18n.format(
                                                    "tooltip.season_seed.fertile_seasons.all"));
                } else {
                    if ((mask & 1) != 0) {
                        event.getToolTip()
                                .add(
                                        TextFormatting.GREEN
                                                + " "
                                                + I18n.format(
                                                        "tooltip.season_seed.fertile_seasons.spring"));
                    }
                    if ((mask & 2) != 0) {
                        event.getToolTip()
                                .add(
                                        TextFormatting.BLUE
                                                + " "
                                                + I18n.format(
                                                        "tooltip.season_seed.fertile_seasons.summer"));
                    }
                    if ((mask & 4) != 0) {
                        event.getToolTip()
                                .add(
                                        TextFormatting.GOLD
                                                + " "
                                                + I18n.format(
                                                        "tooltip.season_seed.fertile_seasons.autumn"));
                    }
                    if ((mask & 8) != 0) {
                        event.getToolTip()
                                .add(
