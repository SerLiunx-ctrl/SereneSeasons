...
public class SeasonColourUtil {
    ...
    public static int applySeasonalGrassColouring(
            ISeasonColorProvider colorProvider, Biome biome, int originalColour) {
        if (!BiomeConfig.enablesSeasonalEffects(biome)
                || !SeasonsConfig.isDimensionWhitelisted(Minecraft.getMinecraft().player.dimension))
        ...
        int newColour =
                overlay == 0xFFFFFF ? originalColour : overlayBlend(originalColour, overlay);
        return saturationMultiplier != -1
                ? saturateColour(newColour, saturationMultiplier)
                : newColour;
    }

    public static int applySeasonalFoliageColouring(
            ISeasonColorProvider colorProvider, Biome biome, int originalColour) {
        if (!BiomeConfig.enablesSeasonalEffects(biome)
                || !SeasonsConfig.isDimensionWhitelisted(Minecraft.getMinecraft().player.dimension))
            return originalColour;
    ...
    private static int getIntFromColour(Color colour) {
        return (colour.getRed() & 255) << 16
                | (colour.getGreen() & 255) << 8
                | colour.getBlue() & 255;
    }
}
