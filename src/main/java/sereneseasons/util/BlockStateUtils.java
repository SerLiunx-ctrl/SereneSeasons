...
public class BlockStateUtils {
    ...
    // returns a set of states, one for every possible combination of values from the provided
    // properties
    public static ImmutableSet<IBlockState> getStatesSet(
            IBlockState baseState, IProperty... properties) {
    ...
    private static void addStatesToList(
            IBlockState state, List<IBlockState> list, Stack<IProperty> stack) {
        if (stack.empty()) {
            list.add(state);
            return;
        } else {
    ...
    // works by looping through all the different values of the properties specified in
    // block.getPresetProperties()
    ...
    /**
     * Discards additional block information to retrieve a state equivalent to those in the
     * inventory*
     */
    ...
    public static IProperty getPropertyByName(IBlockState blockState, String propertyName) {
        for (IProperty property :
                (ImmutableSet<IProperty<?>>) blockState.getProperties().keySet()) {
    ...
    public static Comparable getPropertyValueByName(
            IBlockState blockState, IProperty property, String valueName) {
