...
public class SSCommand extends CommandBase {
    ...
    private void getSeason(ICommandSender sender, String[] args) throws CommandException {
        ...
        int hours =
                (ticksTillNext - (days * ticksPerSecond * 60 * 60 * 24)) / ticksPerSecond / 60 / 60;
        sender.sendMessage(
                new TextComponentTranslation(
                        "commands.sereneseasons.getseason", season, days, hours));
    }
    ...
    private void setSeason(ICommandSender sender, String[] args) throws CommandException {
        ...
        if (newSeason != null) {
            ...
            seasonData.seasonCycleTicks =
                    SeasonTime.ZERO.getSubSeasonDuration() * newSeason.ordinal();
            seasonData.markDirty();
            SeasonHandler.sendSeasonUpdate(player.world);
            sender.sendMessage(
                    new TextComponentTranslation(
                            "commands.sereneseasons.setseason.success", args[1]));
        } else {
            sender.sendMessage(
                    new TextComponentTranslation("commands.sereneseasons.setseason.fail", args[1]));
        }
    }
    ...
    public List<String> getTabCompletions(
        ...
        if (args.length == 1) {
            ...
            return getListOfStringsMatchingLastWord(
                    args,
                    Arrays.stream(SubSeason.values())
