            ...
            if (season == Season.WINTER) {
                if (ModConfig.seasons.changeWeatherFrequency) {
                    ...
                    if (!event.world.getWorldInfo().isRaining()
                            && event.world.getWorldInfo().getRainTime() > 36000) {
                        event.world
                                .getWorldInfo()
                                .setRainTime(event.world.rand.nextInt(24000) + 12000);
                    }
                }
            } else {
                if (ModConfig.seasons.changeWeatherFrequency) {
                    if (season == Season.SPRING) {
                        if (!event.world.getWorldInfo().isRaining()
                            ...
                            event.world
                                    .getWorldInfo()
                                    .setRainTime(event.world.rand.nextInt(84000) + 12000);
                        }
                        ...
                        if (!event.world.getWorldInfo().isThundering()
                            ...
                            event.world
                                    .getWorldInfo()
                                    .setThunderTime(event.world.rand.nextInt(24000) + 12000);
                        }
                    }
                }

                if (ModConfig.seasons.generateSnowAndIce
                        && SeasonsConfig.isDimensionWhitelisted(
                                event.world.provider.getDimension())) {
                    WorldServer world = (WorldServer) event.world;
                    for (Iterator<Chunk> iterator =
                                    world.getPersistentChunkIterable(
                        ...
                        if (world.rand.nextInt(rand) == 0) {
                            ...
                            for (int y = pos.getY(); y >= 0; y--) {
                                Block block =
                                        chunk.getBlockState(pos.getX(), y, pos.getZ()).getBlock();

                                if (block == Blocks.SNOW_LAYER) {
                                    pos = new BlockPos(pos.getX(), y, pos.getZ());
                                    if (SeasonASMHelper.getFloatTemperature(world, biome, pos)
                                            >= 0.15F) {
