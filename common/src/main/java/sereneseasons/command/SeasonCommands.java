...
public class SeasonCommands
{
    ...
    public static void registerArguments(BiConsumer<Identifier, ArgumentTypeInfo<?, ?>> func)
    {
        register(func, "season", SeasonArgument.class, SingletonArgumentInfo.contextFree(SeasonArgument::season));
    }

    private static ArgumentTypeInfo<?, ?> register(BiConsumer<Identifier, ArgumentTypeInfo<?, ?>> func, String name, Class<?> clazz, ArgumentTypeInfo<?, ?> typeInfo)
    {
        func.accept(Identifier.fromNamespaceAndPath(SereneSeasons.MOD_ID, name), typeInfo);
